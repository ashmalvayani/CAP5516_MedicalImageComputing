        ...
        for name, param in sam.named_parameters():
            if param.requires_grad == True:
                ...
                if "image_encoder" in name:
                    image_encoder_params += param.numel()
                if "prompt_encoder" in name:
                    prompt_encoder_params += param.numel()
                if "mask_decoder" in name:
                    mask_decoder_params += param.numel()
        print(f"Image Encoder parameters : {image_encoder_params}")
        print(f"Prompt Encoder parameters : {prompt_encoder_params}")
...
if __name__ == "__main__":
    ...
    # Initialize wandb
    wandb.init(
        project="SAM-Finetune-CrossVal-lora",       # Set your project name here
        name="fold_4",          # You can use a custom run name
        config=args                   # Automatically logs all args to wandb
    )
    ...
    # print(train_img_list, val_img_list, sep='\n\n')
    # exit()
