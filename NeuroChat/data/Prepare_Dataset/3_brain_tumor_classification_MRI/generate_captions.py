...
def generate_caption(labels_path, subset):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        image_path = "/home/parthpk/NeuroChat/Brain Tumor MRI"
        if tumor == "no":
            img_path = f"{image_path}/{subset}/no_tumor/{image}"
        else:
            img_path = f"{image_path}/{subset}/{tumor_type}_tumor/{image}"
        ...
        bat = {
            "custom_id": f'train_{row["image"]}_{row["tumor_type"]}',
            # "custom_id": f"{row['image']}_{row['tumor_type']}_{row['image_type']}",
            "method": "POST", 
    ...
    batc_output_path = "/home/ashmal/Courses/MedImgComputing/NeuroChat/Captions/3_brain_tumor_classification_MRI/TrainBatchFiles"
    ...
    for idx, sub_batch in enumerate(sub_batches):
        with open(f'{batc_output_path}/3_json_batch_{idx}.jsonl', 'w') as f:
...
if __name__ == "__main__":
    labels_path = "/home/parthpk/NeuroChat/Brain Tumor MRI/train_labels_o.csv"
    subset = "Train"
    generate_caption(labels_path, subset)
